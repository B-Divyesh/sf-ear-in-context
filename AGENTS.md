# Ear in Context: an ear-training sandbox with voice-led progressions, sung-pitch keyboard, and no forced progression (`ear-in-context`) — factory product contract

This repository is one product built by the Param Factory. It ships at
`https://ear-in-context.sociobot.in`. Artifact class: `static-web`.
